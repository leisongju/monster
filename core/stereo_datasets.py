...
class MonoDataset(StereoDataset):
    def __init__(self,  mode='train', data_dir='/mnt/datalrl/depth/rgb/unsplash/resize_images', size=(378, 756)):
        self.mode = mode
        self.size = size
        
        # 获取所有样本
        image1_list, image2_list, disp_list = [], [], []
        # 查找所有左图（假设左图文件名为 xxx.jpg）
        left_images = glob.glob(os.path.join(data_dir, "*.jpg"))
        for left_img in left_images:
            base_name = os.path.splitext(os.path.basename(left_img))[0]
            
            for i in range(5):
                right_img = os.path.join(data_dir, f"{base_name}_right_{i}.jpeg")
                disp_img = os.path.join(data_dir, f"{base_name}_disp_{i}.png")
                
                # 如果文件存在，则添加到有效样本
                if os.path.exists(right_img) and os.path.exists(disp_img):
                    image1_list.append(left_img)
                    image2_list.append(right_img)
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        ...
        elif dataset_name == 'mono':
            new_dataset = MonoDataset()
            logging.info(f"Adding {len(new_dataset)} samples from Mono")
